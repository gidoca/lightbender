...
impl GpuBuffer {
    pub unsafe fn new(
        ...
        let memory_type =
            find_memory_type(instance, physical_device, req.memory_type_bits, mem_props)
    ...
    pub unsafe fn upload_slice<T: bytemuck::Pod>(
        ...
        assert!(
            ...
            "upload_slice: data ({} bytes) exceeds buffer size ({})",
            bytes.len(),
            self.size
        );
        unsafe {
...
pub unsafe fn upload_to_device_local<T: bytemuck::Pod>(
    ...
    unsafe { staging.upload_slice(device, data)?; }
